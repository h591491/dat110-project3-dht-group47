...
public class ChordLookup {
	...
	public NodeInterface findSuccessor(BigInteger key) throws RemoteException {
		...
		boolean b = Util.checkInterval(key, node.getNodeID().add(BigInteger.ONE), successor.getNodeID());
		...
		if(b){
			...
			NodeInterface n = findHighestPredecessor(key);
			if(n.getNodeID().equals(this.node.getNodeID())){
				return successor;
			}
	...
	private NodeInterface findHighestPredecessor(BigInteger ID) throws RemoteException {
		...
		for(int i = size - 1; i >= 0; i-- ){
			...
			if (finger != null) {
				...
				boolean b = Util.checkInterval(finger.getNodeID(), node.getNodeID().add(BigInteger.ONE), ID.subtract(BigInteger.ONE));
